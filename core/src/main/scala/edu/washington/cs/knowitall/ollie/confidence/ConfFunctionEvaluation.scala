...
object ConfFunctionEvaluation {
  ...
  def main(args: Array[String]) {
    ...
    System.err.println("Building Feature Set...")

    val featureSet = FeatureSet(OllieExtractionFeatures.getFeatures())
    val exampleSet = new WekaExampleSet(featureSet)
    System.err.println("Loading Labeled Training Examples...")
    val dataInputs = OllieWekaClassifierTrainer.trainingUrls.map(_.openStream())
    ...
    System.err.println("Development Data Size:" + allData.size)
    ...
    println("\n\nAvgConf\tLabel\t" + OllieExtraction.tabDelimitedColumns + "\t" + featureSet.featureNames().mkString("\t"))
    testingRuns.flatten.toList.groupBy(_._1).iterator.map({ tuple =>
      val extr = tuple._1
      val label = tuple._2.head._2
      val confs = tuple._2.map(_._3)
      val avgConf = confs.reduce(_ + _).toDouble / confs.size
      val featureVector = featureSet.getPrintableFeatureVector(extr).mkString("\t")
      (avgConf, label, extr.tabDelimited, featureVector)
  ...
  def trial(allData: List[(OllieExtractionInstance, Int)], featureSet: WekaExampleSet[OllieExtractionInstance]): (List[(OllieExtractionInstance, Int, Double)], List[(OllieExtractionInstance, Int, Double)]) = {
    ...
    val splitIndex = (shuffledData.length / 4) * 3
    ...
    //val confFunction = OllieExtractorClassifierTrainer.trainClassifier(featureSet, trainingData)
    val confFunction = OllieIndependentConfFunction.loadDefaultClassifier
    //println(confFunction.classifier.toString())
  ...
  def getPrecisionYield(extrLabelConfs: List[(OllieExtraction, Int, Double)]): List[(Int, Double)] = {
    ...
    //var totalCorrect = extrLabelConfs.count(_._2 == 1)
    var correctSoFar = 0
    var totalSoFar = 0.0
