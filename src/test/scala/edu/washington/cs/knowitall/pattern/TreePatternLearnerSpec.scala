...
object TreePatternLearnerSpec extends Specification {
  ...
  "no pattern is found" in {
    val row, (rel, arg1, arg2, lemmas, deps) = ("live in", "carol", "new york", Set("carol", "live", "new", "york"), Dependencies.deserialize("advmod(WAS_VBD_2, WHEN_WRB_0); nsubj(WAS_VBD_2, CHRISTOPHER_NNP_1); dobj(WAS_VBD_2, BORN_NNP_3); punct(BORN_NNP_3, _,_4); dobj(WAS_VBD_2, KEN_NNP_5); conj_and(BORN_NNP_3, KEN_NNP_5); nn(LIVED_NNP_8, CAROL_NNP_7); dobj(WAS_VBD_2, LIVED_NNP_8); conj_and(BORN_NNP_3, LIVED_NNP_8); nn(YORK._NNP_11, NEW_NNP_10); prep_in(WAS_VBD_2, YORK._NNP_11)"))
    val graph = DependencyGraph(deps.map(_.lemmatize(MorphaStemmer.instance))).normalize
