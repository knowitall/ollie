...
object BuildPatternsSpec extends Specification {
  ...
  "a pattern is found" in {
    ...
    val patterns = BuildPatterns.findPatterns(graph, lemmas, Map(arg1 -> "arg1", arg2 -> "arg2"), rel, None)
  ...
  "no pattern is found" in {
    ...
    val graph = DependencyGraph.deserialize(pickled).map(_.lemmatize(MorphaStemmer.instance)).normalize
    BuildPatterns.findPatterns(graph, lemmas, Map(arg1 -> "arg1", arg2 -> "arg2"), rel, None) must throwAn[BuildPatterns.NoRelationNodeException]
  }
}
